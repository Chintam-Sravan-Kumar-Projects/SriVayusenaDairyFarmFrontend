...
import {  updateExistingexpenseEntry } from '../../Redux/Slices/expensesSlice';
...
const MoreInfoModal = ({ isOpen, onClose, details, onUpdate ,mode}) => {
  ...
  const handleUpdateClick = () => {
    ...
    dispatch(updateExistingexpenseEntry({id,payload,token})).then(()=>{
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg" >
      ...
      <ModalContent borderRadius={"10px"}>
        <ModalHeader>
          Customer: {details?.farmerId?.name || 'Unknown Farmer'}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Box borderWidth="1px" borderRadius="10px" overflow="hidden" boxShadow="lg">
                  ...
                  <Td>
                    {isEditing ? (
                      <Input
                        ...
                        value={editableDetails.calculatedAmount}
                        onChange={handleInputChange}
                        size="sm"
                        bg={inputBgColor}
                        color={inputTextColor}
            ...
            : <Text color={"Red"} >Please Select Customer from dropdown menu</Text>
            }
          </Box>
        </ModalBody>
        <ModalFooter>
