...
const MilkReportModal = ({ reportData, dateValues }) => {
	...
	const downloadPDF = () => {
		...
		doc.text(`Customer: ${reportData[0].customerId.name || "N/A"}`, 40, 120);
		doc.text(`Mobile: ${reportData[0].customerId.mobile || "N/A"}`, 40, 140);

		doc.text(`Shop Owner: ${reportData[0].adminId.name || "N/A"}`, 350, 120);
		doc.text(`Mobile: ${reportData[0].adminId.mobile || "N/A"}`, 350, 140);
		doc.text(`Email: ${reportData[0].adminId.email || "N/A"}`, 350, 160);
		...
		autoTable(doc, {
			...
			body: [
				[sumLitters, avgRatePerLitter, `Rs: ${sumAmount} `],
			],
		});
		...
		// Download the PDF
		doc.save(`Milk_Report_${reportData[0].customerId.name || "Not Available"}_${new Date().toLocaleDateString()}.pdf`);
	};
