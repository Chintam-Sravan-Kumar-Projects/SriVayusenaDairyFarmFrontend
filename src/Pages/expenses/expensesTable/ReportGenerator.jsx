...
const ExpenseReportModal = ({ reportData, dateValues }) => {
	...
	const downloadPDF = () => {
		...
		doc.text(`Farm Owner: ${reportData[0].adminId.name || "N/A"}`, 40, 100);
		doc.text(`Mobile: ${reportData[0].adminId.mobile || "N/A"}`, 40, 120);
		doc.text(`Email: ${reportData[0].adminId.email || "N/A"}`, 40, 140);
		...
		// Download the PDF
		doc.save(`Expense_Report|| "Not Available"}_${new Date().toLocaleDateString()}.pdf`);
	};
				...
				>
					...
					>
						<Flex
							...
							alignItems={"center"}
						>
							<Heading fontSize={"lg"}>Expense Report Generator</Heading>
							<Button
					...
					<ModalBody>
						...
						<Heading as="h4" size="md" mb={2}>
								{language === "en" ? "Total Expense" : "మొత్తం ఖర్చు"}
							</Heading>
							<Table
								cursor={"pointer"}
								variant="striped"
