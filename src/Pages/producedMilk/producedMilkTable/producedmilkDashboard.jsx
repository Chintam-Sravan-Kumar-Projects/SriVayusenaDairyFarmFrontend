...
import {
	deleteExistingproducedmilkEntry,
	getproducedmilkDetails,
} from "../../../Redux/Slices/producedmilkSlice";
import { toast } from "react-toastify";
import MoreInfoModal from "../../../Components/models/MoreInfoModel2";
import { DeleteConfirmationModal } from "../../../Components/models/DeleteModel";
import ProducedmilkReportModal from "./ReportGenerator";
...
function ProducedmilkDashboard() {
	...
	const [producedmilkStats, setproducedmilkStats] = React.useState([
		{
			fat: 0,
			snf: 0,
			degree: 0,
	...
	// delete Producedmilk entry
	
	const handleDeleteEntry = (item) => {
		
		const id = item._id;
		dispatch(deleteExistingproducedmilkEntry({ id, token }));
	};
	...
	const handleSelectcow = (e) => {
		...
		dispatch(getproducedmilkDetails({ value: payload, token }));
  ...
  const getproducedmilkStats = React.useMemo(() => {
    ...
    return [{
      ...
      totalEntries,
		...
		const topContent = React.useMemo(() => {
			...
			hasSearchFilter,
			dateValues,
			cowData, // add cowData so that it recomputes when cowData changes
		]);
			...
			<div>
				{cowData.length ? (
					<Table aria-label=" static collection table">
						<TableHeader>
							<TableColumn>Cow Name</TableColumn>
						...
						<TableBody
							emptyContent={
								loading || producedmilkStats == [] ? "...Loading" : "No Entry Found"
							}
							items={getproducedmilkStats}
						>
							{(items) => (
...
export default ProducedmilkDashboard;
